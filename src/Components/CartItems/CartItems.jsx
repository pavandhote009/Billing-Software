      ...
      {cartItems.length === 0 ? (
        ...
        <div className='space-y-4'>
          {cartItems.map((item, index) => (
            <div key={index} className='bg-black rounded-lg shadow-sm px-2 h-16 flex flex-col justify-between border border-gray-100'>
              {/* Top row - Name and Price */}
              <div className='flex items-center justify-between'>
                <p className='text-lg font-semibold truncate max-w-[60%]'>{item.name}</p>
                <p className='text-lg font-bold bg-orange-600 px-2 mt-2 rounded-lg text-gray-100'>₹{(item.price * item.quantity).toFixed(2)}</p>
              </div>
              ...
              <div className='flex items-center justify-between '>
                <div className='flex items-center space-x-3 mr-2 '>
                  ...
                  <button
                    onClick={() => updateQuantity(item.itemId, item.quantity + 1)}
                    className='p-1 rounded-full bg-green-500 hover:bg-green-700'
                  >
